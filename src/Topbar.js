...
export default function Topbar({ user, page, onNav, onLogout, onCurrency, onTheme, darkMode, setSbOpen }) {
  const { currency, format } = useCurrency(); // FIXED: use live currency from context
  ...
  const pageTitles = {
    ...
    currencies: 'Currency Rates', massemail: 'Mass Email',
  ...
  const roleColor = user.role === 'admin' ? 'var(--purple)'
    : user.role === 'reseller' ? 'var(--gold)' : 'var(--neon)';
  ...
  const dropItems = [
    ...
    { ic: '💱', lb: `Currency: ${currency.symbol} ${currency.code}`, fn: () => { onCurrency(); setDropOpen(false); } },
    ...
    <>
      <div className="topbar">
        ...
        <div className="topbar-right">
          {/* FIXED: Balance shows in selected currency (PKR by default) */}
          <div className="bal-chip">
            <span className="bc-lbl">Balance</span>
            <span className="bc-val">{format(user.balance)}</span>
          </div>

          {/* Currency switcher button — shows current currency symbol */}
          <button className="btn bgh bsm" onClick={onCurrency}
            style={{ padding: '6px 8px', fontSize: '12px', fontFamily: 'var(--fm)' }} title="Switch Currency">
            {currency.symbol}
          </button>
          ...
          <div ref={dropRef} style={{ position: 'relative' }}>
            ...
            {dropOpen && (
              <div style={{
                position: 'absolute', top: '40px', right: 0, width: '215px',
                ...
                <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--br)', marginBottom: '4px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                    ...
                    {/* FIXED: balance in selected currency */}
                    <span className="bdg b-completed">{format(user.balance)}</span>
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--br)', marginTop: '4px', paddingTop: '4px' }}>
                  <div onClick={() => { onLogout(); setDropOpen(false); }} style={{
                    display: 'flex', alignItems: 'center', gap: '8px',
                    padding: '9px 12px', borderRadius: '7px', cursor: 'pointer',
                    fontSize: '12px', color: 'var(--danger)', fontWeight: 700,
                  }}
      ...
      {telegram && (
        <a href={getTelegramUrl(telegram)} target="_blank" rel="noreferrer"
          style={{
            position: 'fixed', bottom: `calc(var(--mnh) + 136px)`, right: '16px',
            width: '48px', height: '48px', borderRadius: '50%', zIndex: 8000,
            background: '#0088cc', display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '22px', textDecoration: 'none', boxShadow: '0 4px 16px rgba(0,136,204,.5)',
          }}
          onMouseOver={e => e.currentTarget.style.transform = 'scale(1.1)'}
          onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}
          title="Telegram Support">✈️</a>
      )}
      ...
      {whatsapp && (
        <a href={`https://wa.me/${whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
          style={{
            position: 'fixed', bottom: `calc(var(--mnh) + 76px)`, right: '16px',
            width: '48px', height: '48px', borderRadius: '50%', zIndex: 8000,
            background: '#25D366', display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '22px', textDecoration: 'none', boxShadow: '0 4px 16px rgba(37,211,102,.5)',
          }}
          onMouseOver={e => e.currentTarget.style.transform = 'scale(1.1)'}
          onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}
          title="WhatsApp Support">💬</a>
      )}

      {/* Support Floating Button */}
      <button onClick={() => setShowSupport(true)} style={{
        position: 'fixed', bottom: `calc(var(--mnh) + 16px)`, right: '16px',
        width: '48px', height: '48px', borderRadius: '50%', zIndex: 8000,
            ...
            {sent ? (
              ...
              <>
                ...
                <div className="fi">
                  ...
                  <input className="inp" placeholder="e.g. Order not delivered" value={subject} onChange={e => setSubject(e.target.value)} />
                </div>
                <div className="fi">
                  <label className="fl">Message</label>
                  <textarea className="inp" placeholder="Describe your issue..." value={message} onChange={e => setMessage(e.target.value)}
