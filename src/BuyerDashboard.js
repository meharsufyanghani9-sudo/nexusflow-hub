...
// ─── BUG FIX: ORDERS SHOW INCORRECT INFORMATION ON DASHBOARD ─────────────────
// ORIGINAL PROBLEM:
//   The dashboard loads only 5 orders (.limit(5)) for "recent orders" display,
//   then computes stats (total, completed, in_progress, spent) from THOSE 5 rows.
//   So a user with 200 orders sees "Total Orders: 5", "Completed: 3" — wrong!
//
// FIX:
//   Load order STATS (counts + sum) in a separate, unlimited query.
//   Keep the "recent orders" table limited to 5 for display.
//   This way stats are always accurate no matter how many orders the user has.
...
export default function BuyerDashboard({ user, onNav }) {
  ...
  // BUG FIX: separate stats object populated from ALL orders
  const [stats, setStats] = useState({
    total: 0, completed: 0, progress: 0, spent: 0,
  });
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      { data: profile },
      { data: allOrders },    // ALL orders — for accurate stats
      { data: recentOrd },    // latest 5 — for the recent table display
      { data: settingsData },
    ] = await Promise.all([
      supabase.from('users').select('balance').eq('id', user.id).single(),
      supabase.from('orders').select('status, cost').eq('user_id', user.id),
    ...
    // FIXED: compute stats from ALL orders, not just the 5 shown
    if (allOrders) {
      setStats({
        total:     allOrders.length,
        completed: allOrders.filter(o => o.status === 'completed').length,
        progress:  allOrders.filter(o => o.status === 'in_progress').length,
        spent:     allOrders.reduce((a, b) => a + parseFloat(b.cost || 0), 0),
      });
    }
  ...
  return (
    <div>
      ...
      {/* Stats — FIXED: now show accurate counts from ALL orders */}
      <div className="cgrid">
        {[
      ...
      {/* Recent Orders — limited to 5, just for quick overview */}
