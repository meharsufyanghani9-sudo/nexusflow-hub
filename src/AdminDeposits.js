...
export default function AdminDeposits() {
  ...
  const approve = async (dep) => {
    ...
    // FIXED: deposits are stored in PKR (or USDT). We must convert to USD before
    // crediting the balance, since all internal balances are in USD.
    const method = (dep.method || '').toLowerCase();
    ...
    let usdAmount;
    if (isBinance) {
      // Binance is already USDT = USD 1:1
      usdAmount = rawAmount;
    } else {
      // PKR → USD: fetch current PKR rate from currencies table
    ...
    // Transaction record — store USD amount with original currency note
      ...
      if (isFirst) {
        ...
        if (inviter) {
          await supabase.from('users')
            .update({ balance: parseFloat(inviter.balance || 0) + invBonus }).eq('id', inviter.id);
          await supabase.from('transactions').insert({
          ...
          if (joinBonus > 0) {
            await supabase.from('users')
              .update({ balance: newBalance + joinBonus }).eq('id', dep.user_id);
            await supabase.from('transactions').insert({
    ...
    showMsg(`✅ Approved! ${label} → $${usdAmountRounded.toFixed(2)} USD credited to ${dep.user_name || 'user'}.`, 'ok');
  };
              ...
              <div key={d.id} className="card" style={{ padding: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    {/* FIXED: show actual currency (PKR/USDT) not USD */}
