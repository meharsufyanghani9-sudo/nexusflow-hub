 ...
 *
 * WHAT IT DOES:
 *   1. Stores provider credentials (url + api key) in the `settings` table
 *      under keys like "sync_provider_0", "sync_provider_1" etc.
 *   2. On a configurable interval (default 6 hours), fetches the full service
 *      list from every configured provider and compares it with what is in our
 *      `services` table (matched by provider_service_id + provider_api_url).
 *      - NEW service on provider  → INSERT into our services (inherits markup)
 *      - CHANGED rate/min/max     → UPDATE our service
 *      - REMOVED from provider    → marks our service is_active=false and sets
 *        a provider_note so admins know why; does NOT delete (orders reference it)
 *   3. Admin can also trigger a manual sync at any time.
 *   4. Last-sync time and results are stored in settings for display.
 *
 * SUPABASE TABLES USED:
 *   settings  – key/value store for sync config and last-sync metadata
 *   services  – the services table (provider_service_id, provider_api_url, etc.)
 *
 * NO NEW TABLES NEEDED. Everything fits in the existing schema.
 *
 * HOW THE TIMER WORKS:
 *   React's useEffect starts a setInterval when the component mounts.
 *   It reads the interval from settings and fires syncAllProviders().
 *   This means sync only runs while an admin has this page open — which is
 *   fine for most panels. For true background sync (no admin tab needed),
 *   move syncAllProviders() into a Supabase Edge Function or Vercel cron.
 *   The sync logic below is written so it can be copy-pasted into a cron
 *   with zero changes — just swap `fetch('/api/proxy', ...)` for a direct
 *   HTTP call from the server side.
 */
