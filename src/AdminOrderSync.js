 ...
 * orders get stuck in "pending" instead of being handled gracefully.
 *
 * ROOT CAUSE:
 *   1. At order time, if the provider API returns an error (service removed/changed),
 *      the order stays "pending" forever — there is no retry or escalation.
 *   2. Orders with a vendor_order_id (in_progress) are never polled for status
 *      updates, so completions and cancellations from the provider side are missed.
 *
 * THIS FIX:
 *   - "Sync In-Progress Orders": polls provider status API for orders that have a
 *     vendor_order_id, maps provider status → local status, updates DB.
 *   - "Retry Stuck Pending Orders": re-attempts provider API submission for orders
 *     that are still pending AND have a provider_note (meaning first attempt failed).
 *     If the provider returns "service not found / invalid service", the order is
 *     auto-refunded and cancelled with a clear user-facing note.
 *   - Manual override: admin can bulk-force any pending/in_progress orders to a
 *     chosen status with optional refund.
 *
 * SUPABASE COLUMNS REQUIRED (orders table):
 *   - vendor_order_id  TEXT    — provider's order ID (set when provider accepts order)
 *   - provider_note    TEXT    — error/info from provider (shown as ⚠️ tooltip to user)
 *   - provider_api_url TEXT    — stored on the service, joined via service_id
 *   - provider_api_key TEXT    — same
 *   The proxy endpoint /api/proxy must already exist (it does in this project).
 *
 * Note: provider_api_url / provider_api_key come from the services table, not the
 * orders table. We fetch them by joining orders → services at sync time.
 */
