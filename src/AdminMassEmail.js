...
export default function AdminMassEmail() {
  ...
  const sendEmails = async () => {
    ...
    // Fetch target users — FIXED: use 'full_name' not 'name'
          ...
          body: {
            ...
            body: `
              <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;">
                <h2 style="color:#7b2fff;">${subject}</h2>
    ...
    <div style={{ maxWidth: '640px' }}>
      ...
      <div className="card" style={{ padding: '20px', marginBottom: '16px' }}>
        ...
        <div className="fi">
          ...
          <input className="inp" placeholder="e.g. New payment method added!" value={subject} onChange={e => setSubject(e.target.value)} />
        </div>
        <div className="fi">
          <label className="fl">Message Body</label>
          <textarea className="inp" rows={8} style={{ resize: 'vertical', minHeight: '160px' }}
        ...
        {preview && subject && message && (
          <div style={{ padding: '16px', borderRadius: '8px', background: 'rgba(255,255,255,.03)', border: '1px solid var(--br)', marginBottom: '16px' }}>
            <div style={{ fontSize: '10px', color: 'var(--text3)', marginBottom: '10px', letterSpacing: '2px', textTransform: 'uppercase' }}>Email Preview</div>
        ...
        <button className="btn bp blg bw" onClick={sendEmails} disabled={sending || !subject.trim() || !message.trim()}>
