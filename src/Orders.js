import React, { useState, useEffect } from 'react';
import { supabase } from './supabase';

const statusList = ['all','pending','in_progress','completed','cancelled'];

function CountdownTimer({ targetTime, label }) {
  const [timeLeft, setTimeLeft] = useState('');
  const [expired, setExpired] = useState(false);

  useEffect(() => {
    const tick = () => {
      const diff = new Date(targetTime) - new Date();
      if (diff <= 0) { setExpired(true); setTimeLeft('Expired'); return; }
      const m = Math.floor(diff / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      setTimeLeft(`${m}m ${s}s`);
    };
    tick();
    const id = setInterval(tick, 1000);
    return () => clearInterval(id);
  }, [targetTime]);

  if (expired) return null;
  return (
    <span style={{ fontSize: '10px', color: 'var(--gold)', fontFamily: 'var(--fm)', marginLeft: '6px' }}>
      ⏱ {label}: {timeLeft}
    </span>
  );
}

export default function Orders({ user }) {
  const [filter, setFilter] = useState('all');
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [actionMsg, setActionMsg] = useState('');
  // FIX: Track which order is currently being acted on to prevent double-clicks
  const [actingOn, setActingOn] = useState(null);

  useEffect(() => { loadOrders(); }, []);

  const loadOrders = async () => {
    setLoading(true);
    const { data } = await supabase
      .from('orders')
      .select('*')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false });
    if (data) setOrders(data);
    setLoading(false);
  };

  const filtered = orders.filter(o => filter === 'all' || o.status === filter);

  const badgeClass = (s) => {
    if (s === 'completed') return 'b-completed';
    if (s === 'in_progress') return 'b-processing';
    if (s === 'pending') return 'b-pending';
    return 'b-rejected';
  };

  const canCancel = (order) => {
    if (order.status !== 'pending') return false;
    const created = new Date(order.created_at);
    const diff = new Date() - created;
    return diff < 15 * 60 * 1000; // 15 minutes window
  };

  const canRefill = (order) => {
    if (order.status !== 'in_progress') return false;
    const created = new Date(order.created_at);
    const diff = new Date() - created;
    return diff < 24 * 60 * 60 * 1000; // 24 hours window
  };

  const cancelOrder = async (order) => {
    // FIX: Block if already acting on any order
    if (actingOn) return;
    if (!window.confirm('Cancel this order? Your balance will be refunded.')) return;

    setActingOn(order.id);

    // ── FIX 1: IDOR PROTECTION ───────────────────────────────────────────────
    // We now use an atomic update that includes BOTH the order.id AND user_id.
    // This means a user can ONLY cancel their own orders — even if they somehow
    // know another user's order ID they cannot cancel it because the user_id
    // filter will not match.
    // We also check status = 'pending' atomically to prevent double-cancel.
    const { data: cancelledOrder, error: cancelErr } = await supabase
      .from('orders')
      .update({ status: 'cancelled' })
      .eq('id', order.id)
      .eq('user_id', user.id)      // IDOR protection: only your own orders
      .eq('status', 'pending')     // only if still pending (prevents double-cancel)
      .select()
      .single();

    if (cancelErr || !cancelledOrder) {
      setActionMsg('❌ Could not cancel — order may have already been processed.');
      setActingOn(null);
      setTimeout(() => setActionMsg(''), 4000);
      return;
    }
    // ────────────────────────────────────────────────────────────────────────

    // ── FIX 2: Read LIVE balance before adding refund ─────────────────────
    // Use the cost from the DB record we just cancelled (not from local state)
    const { data: profile } = await supabase
      .from('users')
      .select('balance')
      .eq('id', user.id)
      .single();

    if (profile) {
      const refundAmount = parseFloat(cancelledOrder.cost || 0);
      const newBalance = parseFloat(profile.balance) + refundAmount;
      await supabase
        .from('users')
        .update({ balance: newBalance })
        .eq('id', user.id);

      // Log the refund in transactions
      await supabase.from('transactions').insert({
        user_id: user.id,
        type: 'refund',
        amount: refundAmount,
        description: `Refund: Order ${cancelledOrder.order_ref || order.id}`,
        ref_id: cancelledOrder.order_ref || order.id,
      });
    }

    setActionMsg('✅ Order cancelled. Your balance has been refunded!');
    setActingOn(null);
    loadOrders();
    setTimeout(() => setActionMsg(''), 4000);
  };

  const refillOrder = async (order) => {
    if (actingOn) return;
    if (!window.confirm('Request a refill for this order?')) return;

    setActingOn(order.id);

    // FIX: Include user_id guard so users can only refill their own orders
    await supabase
      .from('orders')
      .update({ refill_requested: true })
      .eq('id', order.id)
      .eq('user_id', user.id);   // IDOR protection

    setActionMsg('✅ Refill requested! Admin will process it shortly.');
    setActingOn(null);
    loadOrders();
    setTimeout(() => setActionMsg(''), 4000);
  };

  const stats = {
    total: orders.length,
    completed: orders.filter(o => o.status === 'completed').length,
    progress: orders.filter(o => o.status === 'in_progress').length,
    pending: orders.filter(o => o.status === 'pending').length,
  };

  return (
    <div>
      {actionMsg && (
        <div style={{
          background: actionMsg.startsWith('✅') ? 'rgba(0,255,136,.08)' : 'rgba(255,50,80,.08)',
          border: `1px solid ${actionMsg.startsWith('✅') ? 'rgba(0,255,136,.2)' : 'rgba(255,50,80,.2)'}`,
          borderRadius: '8px', padding: '12px', textAlign: 'center',
          color: actionMsg.startsWith('✅') ? 'var(--green)' : '#ff6b6b',
          fontWeight: 700, marginBottom: '16px', fontSize: '13px'
        }}>
          {actionMsg}
        </div>
      )}

      {/* Filter Tabs */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
        {statusList.map(s => (
          <button key={s} onClick={() => setFilter(s)}
            style={{
              padding: '6px 14px', borderRadius: '20px', cursor: 'pointer',
              fontFamily: 'var(--fu)', fontSize: '11px', fontWeight: 700,
              textTransform: 'uppercase', letterSpacing: '1px', transition: '.15s',
              background: filter === s ? 'var(--neon)' : 'var(--gl)',
              color: filter === s ? '#000' : 'var(--text3)',
              border: filter === s ? 'none' : '1px solid var(--br)',
            }}>
            {s.replace('_',' ')}
          </button>
        ))}
      </div>

      {/* Stats Row */}
      <div className="cgrid" style={{ marginBottom: '16px' }}>
        {[
          { ic:'📦', lb:'Total', vl: stats.total, cl:'cn' },
          { ic:'✅', lb:'Completed', vl: stats.completed, cl:'cg' },
          { ic:'⚡', lb:'In Progress', vl: stats.progress, cl:'cn' },
          { ic:'⏳', lb:'Pending', vl: stats.pending, cl:'cw' },
        ].map((s,i) => (
          <div key={i} className="sc">
            <span className="sc-ic">{s.ic}</span>
            <div className="sc-lb">{s.lb}</div>
            <div className={`sc-vl ${s.cl}`}>{s.vl}</div>
          </div>
        ))}
      </div>

      {/* Orders Table */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text3)' }}>Loading orders...</div>
      ) : (
        <div className="tblw">
          <table>
            <thead>
              <tr>
                <th>#ID</th>
                <th>Service</th>
                <th>Qty</th>
                <th>Cost</th>
                <th>Status</th>
                <th>Date</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan="7" style={{ textAlign:'center', padding:'32px', color:'var(--text3)' }}>
                    No orders found
                  </td>
                </tr>
              ) : filtered.map(o => (
                <tr key={o.id}>
                  <td style={{ fontFamily:'var(--fm)', color:'var(--neon)', fontSize:'11px' }}>
                    {o.order_ref || o.id?.slice(0,8)}
                  </td>
                  <td style={{ fontWeight:600, maxWidth:'120px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {o.service_name}
                  </td>
                  <td style={{ fontFamily:'var(--fm)' }}>{o.quantity?.toLocaleString()}</td>
                  <td style={{ color:'var(--gold)', fontFamily:'var(--fm)' }}>
                    ${parseFloat(o.cost||0).toFixed(2)}
                  </td>
                  <td>
                    <span className={`bdg ${badgeClass(o.status)}`}>
                      {o.status?.replace('_',' ')}
                    </span>
                    {canCancel(o) && (
                      <CountdownTimer
                        targetTime={new Date(new Date(o.created_at).getTime() + 15*60*1000)}
                        label="Cancel in"
                      />
                    )}
                    {o.vendor_order_id && (
                      <div style={{ fontSize:'9px', color:'var(--text3)', marginTop:'2px' }}>
                        Vendor: {o.vendor_order_id}
                      </div>
                    )}
                  </td>
                  <td style={{ color:'var(--text3)', fontSize:'10px' }}>
                    {new Date(o.created_at).toLocaleDateString()}
                  </td>
                  <td>
                    <div style={{ display:'flex', gap:'4px', flexWrap:'wrap' }}>
                      {canCancel(o) && (
                        // FIX: Button is disabled while actingOn to prevent double-click
                        <button
                          onClick={() => cancelOrder(o)}
                          disabled={actingOn === o.id}
                          style={{
                            padding:'4px 10px', borderRadius:'6px',
                            border:'1px solid rgba(255,50,50,.3)',
                            background: actingOn === o.id ? 'rgba(100,100,100,.15)' : 'rgba(255,50,50,.1)',
                            color: actingOn === o.id ? 'var(--text3)' : '#ff6b6b',
                            cursor: actingOn === o.id ? 'not-allowed' : 'pointer',
                            fontSize:'11px', whiteSpace:'nowrap'
                          }}>
                          {actingOn === o.id ? '⏳' : 'Cancel'}
                        </button>
                      )}
                      {canRefill(o) && !o.refill_requested && (
                        <button
                          onClick={() => refillOrder(o)}
                          disabled={actingOn === o.id}
                          style={{
                            padding:'4px 10px', borderRadius:'6px',
                            border:'1px solid rgba(0,255,136,.3)',
                            background: actingOn === o.id ? 'rgba(100,100,100,.15)' : 'rgba(0,255,136,.1)',
                            color: actingOn === o.id ? 'var(--text3)' : 'var(--green)',
                            cursor: actingOn === o.id ? 'not-allowed' : 'pointer',
                            fontSize:'11px', whiteSpace:'nowrap'
                          }}>
                          {actingOn === o.id ? '⏳' : 'Refill'}
                        </button>
                      )}
                      {o.refill_requested && (
                        <span style={{ fontSize:'10px', color:'var(--gold)' }}>⏳ Refill pending</span>
                      )}
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
