...
export default function PanelApi({ user }) {
  ...
  const loadMyKeys = async () => {
    setLoading(true);
    // FIX #9: query by user_id (UUID) not label (email string)
  ...
  const generateAndSave = async () => {
    ...
    // FIX #9: store user_id instead of relying solely on label
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <div style={{
        ...
        border: '1px solid rgba(0,212,255,.2)', marginBottom: '20px',
      }}>
        ...
        <div style={{ fontSize: '12px', color: 'var(--text2)', lineHeight: 1.7 }}>
          Use our API to integrate NexusFlow services into your own panel or website.
          Standard SMM panel API format compatible with most platforms.
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '14px', marginBottom: '16px' }}>
        ...
        <div style={{ fontSize: '11px', color: 'var(--text3)' }}>
          POST requests · JSON responses · Standard SMM panel format
        </div>
      </div>
        ...
        <>
          ...
          {genKey && (
            <div style={{
              ...
              <div style={{ fontSize: '10px', color: 'var(--green)', marginBottom: '4px', fontWeight: 700 }}>
                ✅ New key generated — save it now!
              </div>
              <div
                style={{ fontFamily: 'var(--fm)', fontSize: '11px', color: 'var(--green)', wordBreak: 'break-all', cursor: 'pointer' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {codeExamples.map((ex, i) => (
          <div key={i} className="card" style={{ padding: '14px' }}>
            <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--neon)', marginBottom: '8px' }}>
              {ex.title}
            </div>
            <div
              style={{
                fontFamily: 'var(--fm)', fontSize: '10px', color: 'var(--text2)',
                background: 'rgba(0,0,0,.4)', padding: '10px 12px', borderRadius: '6px',
                whiteSpace: 'pre-wrap', wordBreak: 'break-all', lineHeight: 1.8, cursor: 'pointer',
              }}
