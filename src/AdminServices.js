    ...
    <div>
      {msg && !modal && (
        <div style={{
          ...
          border: `1px solid ${msg.startsWith('✅') || msg.startsWith('⭐') ? 'rgba(0,255,136,.2)' : msg.startsWith('🗑') ? 'rgba(255,100,0,.2)' : 'rgba(255,50,80,.2)'}`,
      ...
      <div style={{ background: 'rgba(0,212,255,.06)', border: '1px solid var(--br)', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px', fontSize: '11px', color: 'var(--text2)', lineHeight: 1.8 }}>
        <strong style={{ color: 'var(--neon)' }}>⭐ Featured Services</strong> appear prominently on the Marketplace page. All other active services are hidden behind "Browse All".<br />
        <strong style={{ color: 'var(--gold)' }}>🔌 API-linked services</strong> automatically place orders with the provider the moment a user orders — no manual work needed.
      </div>
          ...
          <div className="mbox" onClick={e => e.stopPropagation()}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
              ...
              <button onClick={() => setModal(false)}
                style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: '20px' }}>×</button>
            </div>
            ...
            <div style={{ fontSize: '9px', color: 'var(--text3)', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '10px', marginTop: '6px' }}>Pricing & Quantity</div>
