...
export default function AdminDashboard({ user, onNav }) {
  ...
  const loadData = async () => {
    ...
    const [
      ...
      { count: resellers },
      { count: openDisputes },
      { count: liveServices },     // FIXED: query is_active not status
      { data: ordersData },
      { data: depositsData },
      { data: settingsData },
    ] = await Promise.all([
      ...
      supabase.from('services').select('*', { count: 'exact', head: true }).eq('is_active', true), // FIXED
      supabase.from('orders').select('cost'),
      supabase.from('deposits').select('amount, status, user_id, user_name, created_at').order('created_at', { ascending: false }).limit(5),
      supabase.from('settings').select('*'),
    ]);

    const revenue = (ordersData || []).reduce((a, b) => a + parseFloat(b.cost || 0), 0);

    setStats({
      totalUsers: totalUsers || 0, totalOrders: totalOrders || 0,
      revenue, pendingDeposits: pendingDeposits || 0,
      resellers: resellers || 0, openDisputes: openDisputes || 0,
      withdrawals: 0, liveServices: liveServices || 0,
    });
  ...
  return (
    <div>
      ...
      {announcement && (
        <div style={{ background: 'linear-gradient(90deg,rgba(0,212,255,.12),rgba(123,47,255,.12))', border: '1px solid rgba(0,212,255,.25)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '18px' }}>📢</span>
          <span style={{ fontSize: '13px', color: 'var(--text)', flex: 1 }}>{announcement}</span>
          <button onClick={() => setAnnouncement('')} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: '18px' }}>×</button>
        </div>
      )}

      <div className="bhr mb20" style={{ background: 'linear-gradient(135deg,rgba(123,47,255,.2),rgba(0,20,60,.35),rgba(40,0,80,.2))' }}>
        <div className="bh-lbl">Admin Panel</div>
        <div className="bh-amt" style={{ fontSize: 'clamp(20px,4vw,32px)' }}>Welcome, {user.name} 👑</div>
        <div className="bh-ft">
          <div style={{ fontFamily: 'var(--fd)', fontSize: '10px', letterSpacing: '2px', color: 'rgba(255,255,255,.4)' }}>NEXUSFLOW ADMIN</div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '30px', color: 'var(--text3)' }}>Loading stats...</div>
      ) : (
        <div className="cgrid">
          {statCards.map((s, i) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: '10px', marginBottom: '20px' }}>
        {quickActions.map((a, i) => (
          <div key={i} className="card" style={{ padding: '16px', cursor: 'pointer', transition: 'all .2s' }}
            onClick={() => onNav(a.pg)}
        ...
        <table>
          ...
          <tbody>
            {recentActivity.length === 0 ? (
              <tr><td colSpan="4" style={{ textAlign: 'center', padding: '32px', color: 'var(--text3)' }}>No activity yet</td></tr>
            ) : recentActivity.map((d, i) => (
              <tr key={i}>
                <td style={{ fontFamily: 'var(--fm)', fontSize: '11px', color: 'var(--neon)' }}>
                  {d.user_name || d.user_id?.slice(0, 8) + '...'}
                </td>
                <td style={{ color: 'var(--gold)', fontFamily: 'var(--fm)' }}>${parseFloat(d.amount || 0).toFixed(2)}</td>
                <td>
                  <span className={`bdg ${d.status === 'approved' ? 'b-completed' : d.status === 'pending' ? 'b-pending' : 'b-rejected'}`}>{d.status}</span>
                </td>
                <td style={{ color: 'var(--text3)', fontSize: '10px' }}>{new Date(d.created_at).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
